        ...
        for n=1:length(allFiles)
            ...
            if ~isempty(temp) %dicom file
                ...
                if check
                    ...
                    if isfield(temp,'KVP')
                        kVp(sliceCounter) = single(temp.KVP);
                    end
                    if isfield(temp,'XrayTubeCurrent')
                        tubeCurrent(sliceCounter) = single(temp.XrayTubeCurrent);
                    end
                    if isfield(temp,'ExposureTime')
                        exposureTime(sliceCounter) = single(temp.ExposureTime);
                    end
                    if isfield(temp,'SpiralPitchFactor')
                        pitch(sliceCounter) = single(temp.SpiralPitchFactor);
                    end
                    if isfield(temp,'Exposure')
                        exposure(sliceCounter) = single(temp.Exposure);
                    end
